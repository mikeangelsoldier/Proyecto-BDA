...
CREATE PROC  SP_LLENA_VEHICULO
  ...
  begin tran llena_vehiculo
	 DECLARE @ID_VEHICULO VARCHAR(15),  @MODELO VARCHAR(300), @TIPO VARCHAR(300), @AÑO INT, @COLOR VARCHAR(200), @PRECIO MONEY,
      	@CTA bigint , @cantidad_modelos int, @cantidad_tipos bigint, @cantidad_colores int,
      	@CONT BIGINT = 1
	 ...
	 WHILE (@CONT <=  @N)
        BEGIN    
			...
			INSERT INTO VEHICULO (ID_VEHICULO,DECRIPCION,TIPO,AÑO,MODELO,COLOR,PRECIO)
			VALUES(@ID_VEHICULO,'Vehiculo',@TIPO,@AÑO,@MODELO,@COLOR, @PRECIO)
