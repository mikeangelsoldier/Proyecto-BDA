...
SELECT * FROM DBO.fn_ventas_vehiculos(2,2017)--vehiculos vendidos en el MES XX, ANIO XXXX
GO
...
*/
/*
SELECT * FROM VENTA_VEHICULO vv JOIN VEHICULO v ON vv.FK_VEHICULO=v.ID_VEHICULO ORDER BY MODELO
*/
/*
PRUEBA   
...
CREATE FUNCTION FN_SUMA_TOTALES(@MES SMALLINT, @ANIO SMALLINT,@MODELO VARCHAR(300),@TIPO VARCHAR(300), @COLOR VARCHAR(200))
	RETURNS TABLE
	AS 
	RETURN(SELECT FK_VEHICULO
			FROM VENTA_VEHICULO WHERE ID_VENTA_VEHICULO IN
...
SELECT * FROM DBO.FN_SUMA_TOTALES (3,2017,'PLATINA','estándar','negro amatista metalizado') 
