...
DELETE FROM DIMENSION_VEHICULO
...
SELECT ds.ESTADO, SUM(PRECIO) GANANCIA_TOTAL FROM TABLA_HECHO_VENTAS_VEHICULO thvv JOIN DIMENSION_CLIENTE dc
	ON thvv.ID_CLIENTE=dc.ID_CLIENTE JOIN DIMENSION_FECHA df
	ON thvv.ID_FECHA=df.ID_FECHA JOIN DIMENSION_SUCURSAL ds
	ON thvv.ID_SUCURSAL=ds.ID_SUCURSAL JOIN DIMENSION_VEHICULO dv
	ON thvv.ID_VEHICULO=dv.ID_VEHICULO
	GROUP BY ds.ESTADO
...
CREATE PROC  SP_LLENA_TABLA_HECHO_VENTAS_VEHICULO
  ...
  AS
	 ...
	 @ID_VEHICULO VARCHAR(15),  @TIPO VARCHAR(300), @AÑO_COCHE INT, @MODELO VARCHAR(300), @COLOR VARCHAR(200), @PRECIO MONEY, @ESTADO_COCHE VARCHAR(20),
	 @cantidad_vechilos BIGINT,
      	
	 @ID_CLIENTE VARCHAR(15),  @NOMBRE VARCHAR(100),  @PRIMER_APELLIDO VARCHAR(100),  @SEGUNDO_APELLIDO VARCHAR(100), @FECHA_NACIMIENTO DATE, @SEXO VARCHAR(1),
	
     @ID_SUCURSAL VARCHAR(15),  @DESCRIPCION VARCHAR(300),  @ESTADO VARCHAR(30),  @CIUDAD VARCHAR(100), @PUNTUACION INT,
	
	 @ID_FECHA VARCHAR(15),@NUM_VENTA VARCHAR(30),@FECHA DATE,
	 @DIA_DEL_MES INT,@MES INT, @AÑO INT, @SEMANA_DEL_AÑO INT,
