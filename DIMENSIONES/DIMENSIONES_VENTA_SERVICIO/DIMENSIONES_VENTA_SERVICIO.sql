...
CREATE VIEW Det_Fac_ServicioPPP
AS
SELECT ROW_NUMBER() OVER (ORDER BY FK_FACT_SERVICIO) AS INDICE, FK_FACT_SERVICIO, FK_SERVICIO 
 FROM DET_FACT_SERVICIO
GO

SELECT * FROM Det_Fac_ServicioPPP
GO
...
CREATE VIEW VENTAS_SERVICIO_PPP
AS
SELECT ROW_NUMBER() OVER (ORDER BY ser.ID_FACT_SERVICIO) AS INDICE,
se.ID_SERVICIO, s.ID_SUCURSAL, c.ID_CLIENTE, FECHA, ser.ID_FACT_SERVICIO  FROM DISTRIBUIDOR_O_SUCURSAL as s INNER JOIN FACT_SERVICIO ser ON ser.FK_DISTRIBUIDORA = s.ID_SUCURSAL
JOIN CLIENTE c ON c.ID_CLIENTE = ser.FK_CLIENTE JOIN DET_FACT_SERVICIO det_ser ON ser.ID_FACT_SERVICIO = det_ser.FK_FACT_SERVICIO JOIN SERVICIO se ON det_ser.FK_SERVICIO = se.ID_SERVICIO
GO

SELECT * FROM VENTAS_SERVICIO_PPP
GO
...
ALTER TABLE DIMENSION_SERVICIO_S  ADD 
	CONSTRAINT PK_VEHICULO_DIMENSION_VEHICULO_S
...
CREATE PROC SP_LLENA_DIMENSION_SERVICIO
  AS
	 ...
	 DECLARE @ID_SERVICIO VARCHAR(15), @DESCRIPCION VARCHAR (500), @PRECIO MONEY, @PUNTUACION INT,
	@cantidad_servicios BIGINT,
      	
	 @CONT BIGINT = 1 , @CONT2 BIGINT=(SELECT COUNT(FK_SERVICIO) FROM DET_FACT_SERVICIO)
	 ...
	 WHILE (@CONT <=  @CONT2)
	    ...
	    SELECT  @ID_SERVICIO = FK_SERVICIO 
				FROM Det_Fac_ServicioPPP
				WHERE INDICE = @CONT
		...
		SELECT  @PUNTUACION = PUNTUACION
				FROM SERVICIO INNER JOIN DET_FACT_SERVICIO
				ON @ID_SERVICIO = FK_SERVICIO
				WHERE ID_SERVICIO = @ID_SERVICIO
				
				INSERT INTO DIMENSION_SERVICIO_S VALUES(@ID_SERVICIO,@DESCRIPCION,@PRECIO,@PUNTUACION);
...
CREATE PROC  SP_LLENA_TABLA_HECHO_VENTAS_SERVICIO
  ...
  AS
	 ...
	 DECLARE @ID_FECHA VARCHAR(15), @ID_SUCURSAL VARCHAR(15),@ID_CLIENTE VARCHAR(15), @ID_SERVICIO VARCHAR(15),@NUM_VENTA VARCHAR(30),@FECHA DATE,
	 ...
	 WHILE (@CONT <=  @N)
		...
		SELECT  @NUM_VENTA = ID_FACT_SERVICIO
				FROM VENTAS_SERVICIO_PPP
				WHERE INDICE = @CONT
		...
		INSERT INTO TABLA_HECHO_VENTAS_SERVICIO VALUES (@ID_FECHA,@ID_VEHICULO,@ID_SUCURSAL,@ID_CLIENTE,@NUM_VENTA)
